...
from app.exceptions import IncorrectEmailOrPasswordFormatException, UserAlreadyExistsException
...
@router.post("/login")
async def register_user(responce: Response, user_data: SUserAuth):
    user = await authenticate_user(user_data.email, user_data.password)
    if not user:
        raise IncorrectEmailOrPasswordFormatException
